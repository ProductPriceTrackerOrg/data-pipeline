...
class LifeMobileSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.error_count = 0
        self.max_products = 60000
        self.max_runtime = 7200  # 2 hours maximum runtime
    ...
    def parse_category(self, response):
        ...
        if response.status in [404, 503, 429] or self.products_collected >= self.max_products:
            self.error_count += 1
            return
        ...
        if depth > 15:  # Limit pagination depth
            return
        ...
        for selector in product_selectors:
            ...
            for url in product_links:
                if (url and self.products_collected < self.max_products and 
                    "/product/" in url):
                    full_url = self.normalize_url(urljoin(response.url, url))
        ...
        # Handle pagination - only follow if we haven't collected enough products
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            price_original = response.css(
                "p.price del span.amount bdi::text"
            ).get(default="").strip()

            discount_percentage = response.css("span.percentage::text").get()
            if discount_percentage:
                discount_percentage = discount_percentage.strip().replace("%", "")

            specs = {}
            spec_items = response.css(
                "div.woocommerce-product-details__short-description li, div.aps-feature-info div"
            )
            ...
            payment_options = []
            payment_elements = response.css("div.mayapgs-payment")
            for payment in payment_elements:
                ...
                if payment_name and payment_price:
                    payment_options.append(
                        {"provider": payment_name, "price": payment_price}
                    )

            variants_data = response.css("form.variations_form::attr(data-product_variations)").get()
            variants = []
            ...
            if variants_data:
                try:
                    ...
                    for variant in variants_json:
                        ...
                        # Discount calculation for variants
                        if discount_percentage and variant_price_original:
                            try:
                                original_value = float(
                                    re.sub(r"[^\d.]", "", variant_price_original)
                                )
                                ...
                                variant_price_current = (
                                    variant_price_current or variant_price_original
                                )
                                variant_price_original = (
                                    variant_price_original or variant_price_current
                                )
                        else:
                            variant_price_current = (
                                variant_price_current or variant_price_original
...
if __name__ == "__main__":
    process = CrawlerProcess(
        settings={
            "FEED_FORMAT": "json",
            "FEED_URI": "lifemobile_products.json",
            "LOG_LEVEL": "INFO",
            "CONCURRENT_REQUESTS": 100,
            "CONCURRENT_REQUESTS_PER_DOMAIN": 20,
            "DOWNLOAD_DELAY": 0.02,
